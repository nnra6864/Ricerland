...
SOURCE_FILE="$SCRIPT_PATH/autoexec.cfg"
...
CONFIG_DIR="$STEAM_LIBRARY_DIR/steamapps/common/Counter-Strike Global Offensive/game/csgo/cfg"
FONT_DIR="$STEAM_LIBRARY_DIR/steamapps/common/Counter-Strike Global Offensive/game/csgo/panorama/fonts"
CONF_D_DIR="$STEAM_LIBRARY_DIR/steamapps/common/Counter-Strike Global Offensive/game/core/panorama/fonts/conf.d"
...
MOVEMENT_CONFIG_PATH="$CONFIG_DIR/cfg.zip"
...
unzip -q -o "$MOVEMENT_CONFIG_PATH" -d "$CONFIG_DIR"
if [ $? -ne 0 ]; then
    echo "Error: Failed to unzip '$MOVEMENT_CONFIG_PATH'"
    exit 1
fi
echo "Successfully extracted the movement config into '$CONFIG_DIR/movement'"
rm -rf "$MOVEMENT_CONFIG_PATH"
echo "Successfully deleted '$MOVEMENT_CONFIG_PATH'"

print_separator

echo "exec movement/setup" > "$CONFIG_DIR/listenserver.cfg"
echo "Successfully created '$CONFIG_DIR/listenserver.cfg'"

print_separator

FULL_LINK_PATH="$CONFIG_DIR/$CONFIG_NAME"
if [ -e "$FULL_LINK_PATH" ]; then
    read -p "A file or link named '$CONFIG_NAME' already exists in '$CONFIG_DIR'. Overwrite? (y/N): " confirm_overwrite
...
read -p "Install the custom font? (Y/n): " install_font
if [[ ! "$install_font" =~ ^[Nn]$ ]]; then
    echo "Skipping custom font installation"
else
    echo "Installing the custom font"
    if [ ! -d "$FONT_DIR" ]; then
        echo "Error: Font directory '$FONT_DIR' does not exist"
        echo "Please ensure the Steam Library path is correct and CS2 is installed in that library"
        exit 1
    fi

    if [ ! -d "$CONF_D_DIR" ]; then
        echo "Error: conf.d directory '$CONF_D_DIR' does not exist"
        echo "Please ensure the Steam Library path is correct and CS2 is installed in that library"
        exit 1
    fi

    for f in "$FONT_DIR"/*; do
        [ "$(basename "$f")" != ".uuid" ] && rm -rf -- "$f"
    done
fi
