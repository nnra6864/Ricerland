...
class C3DB_CursorService:
    ...
    @staticmethod
    def convert_selected_to_empty(context) -> Optional[bpy.types.Object]:
        scene = context.scene
        coll = scene.C3DB_3Dcursors_collection
        index = scene.C3DB_3Dcursors_index
        if not _ensure_valid_index(index, len(coll)):
            return None
        return C3DB_CursorService.convert_cursor3d_to_empty(context, coll[index])
...
class C3DB_MT_menu_specials(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.separator()
        icon = "HIDE_OFF" if prefs.show_icons else "HIDE_ON"
        layout.prop(prefs, "show_icons", text="Show Controls as Icons", icon=icon)
...
class C3DB_OT_save(bpy.types.Operator):
    ...
    def execute(self, context):
        C3DB_CursorService.save_current_cursor(context.scene)
...
class C3DB_OT_remove_from_list(bpy.types.Operator):
    ...
    def execute(self, context):
        C3DB_CursorService.remove_at_index(context.scene, context.scene.C3DB_3Dcursors_index)
        context.area.tag_redraw()
        return {"FINISHED"}
...
class C3DB_OT_convert_selected_to_empty(bpy.types.Operator):
    """Convert selected 3D Cursor to an empty object"""
